...
class LSFMDistortionBeadsDataset(Dataset):
    ...
    def _apply_asymmetric_ri(
        ...
        coeffs = torch.empty(4).uniform_(-s, s, generator=g)
        ax, ay, az, axy = coeffs
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        sample: dict[str, torch.Tensor] = {
            "stack_distorted": distorted.unsqueeze(0).float(),
            "stack_corrected": corrected.unsqueeze(0).float(),
            "bead_centers": distorted_centers.float(),
            "undistorted_bead_centers": centers.float(),
