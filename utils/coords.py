"""Coordinate helpers for the INR decoder."""
...
def make_voxel_grid(
    ...
    """Dense (Nz*Ny*Nx, 3) grid in [-1, 1]^3 with order (z, y, x)."""
    ...
    return torch.stack([Z, Y, X], dim=-1).reshape(-1, 3)         # (M, 3)


def sample_voxel_coords(
    n_samples: int, device: torch.device | str = "cpu"
) -> torch.Tensor:
    """Uniform random coords in [-1, 1]^3."""
